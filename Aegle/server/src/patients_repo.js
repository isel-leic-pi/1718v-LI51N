...
function createRepository (events) {
  ...
  const getStatus = (patientId) => {
    ...
    if (!patient) { return new model.PatientStatus(patientId, 'UNKNOWN') }

    const heartbeats = patient.events.get('Heartbeat')
    if (!heartbeats || heartbeats.length === 0) { return new model.PatientStatus(patientId, 'UNKNOWN') }
  ...
  const addEvent = (event) => {
    ...
    if (!eventList) { patient.events.set(event.type, eventList = []) }

    eventList.push(event)
  }

  if (events && events instanceof Array) { events.forEach((event) => addEvent(event)) }
  ...
  return {
    ...
    updatePatient: (patient, cb) => {
      ...
      if (patient.heartRate) { existingPatient.patientData.heartRate = patient.heartRate }
